...
function wcpbc_set_woocommerce_country( $country ) {
	...
	if ( ! wcpbc_is_woocommerce_frontend() || ! in_array( $country, array_keys( WC()->countries->countries ) ) ) {
		return;
	}
...
function wcpbc_sync_exchange_rate_prices( $zone_id, $exchange_rate ){
	...
	$parent_product_ids = $wpdb->get_col( $wpdb->prepare( "
		...
		INNER JOIN {$wpdb->posts} p ON p.ID = t_r.object_id and p.post_type = 'product'
 ...
 *
 * @since 1.7.0
 * @param string $basic_or_pro 
 * @param string $context 
 * return boolean
 */
function wcpbc_product_types_supported( $basic_or_pro = '', $context = '' ) {	
	...
	if ( empty( $basic_or_pro ) ) {
		$types = array_merge( $types['basic'], $types['pro'] );
	} elseif( $basic_or_pro === 'pro' ) {
		$types = $types['pro'];
	} else {
		$types = $types['basic'];
	}
