...
class WCPBC_Admin_Product_Data {
	...
	public static function product_options_countries_prices() {					
		...
		foreach ( WCPBC()->get_regions() as $key => $value ) {	
			...
			?>
				<div class="options_group pricing show_if_simple show_if_external wcpbc_pricing<?php if ( ! wcpbc_is_pro() ) : foreach ( array_keys( wcpbc_product_types_supported( 'pro', 'product-data' ) ) as $product_type) : echo ' hide_if_' . $product_type; endforeach; endif; ?>">					
					...
					<div style="display: <?php echo ($_price_method == 'exchange_rate' ? 'none' : 'block' ); ?>" class="wcpbc_show_if_manual">
						...
						<?php do_action('wc_price_based_country_after_product_options_pricing', $_id_prefix, $value['currency'] ); ?>
	...
	public static function process_product_simple_countries_prices( $post_id, $i = false ) {				
		...
		foreach ( WCPBC()->get_regions() as $key => $value ) {
			...
			if ( $_price_method == 'exchange_rate' ) {
				...
				if ( ! empty( $_sale_price ) && empty( $_sale_price_dates_to ) && empty( $_sale_price_dates_from ) ) {
					$_price = $_sale_price;
				} elseif ( ! empty( $_sale_price ) && $_sale_price_dates_from && $_sale_price_dates_from <= strtotime( 'NOW', current_time( 'timestamp' ) ) ) {
					$_price = $_sale_price;					
				} else {
	...
	public static function product_variable_attributes_countries_prices( $loop, $variation_data, $variation ) {							
		...
		foreach ( WCPBC()->get_regions() as $key => $value) {
			...
			?>
				<div class="wcpbc_pricing<?php if ( ! wcpbc_is_pro() && ! class_exists( 'WCPBC_Subscriptions' ) ) echo ' hide_if_variable-subscription'; ?>">					
	...
	public static function variable_product_bulk_edit_actions(){
		...
		add_action( 'admin_footer', array( 'WCPBC_Admin', 'upgrade_pro_popup' ) );			
	}
	...
	public static function product_quick_bulk_edit_save( $product ) {
		...
		foreach ( WCPBC()->get_regions() as $key => $value ) {
			...
			$parent_id    = version_compare( WC_VERSION, '3.0', '<' ) ? $product->get_parent() : $product->get_parent_id();			
			$product_type = version_compare( WC_VERSION, '3.0', '<' ) ? $product->product_type : $product->get_type();			

			$price_method =  get_post_meta( $product_id, "_{$key}_price_method", true );
						
			if ( $price_method == 'exchange_rate' || empty( $price_method ) ) {				
				
				$regular_price 	= get_post_meta( $product_id, '_regular_price', true );
				$sale_price 	= get_post_meta( $product_id, '_sale_price', true );
				$price 			= get_post_meta( $product_id, '_price', true );
				
				update_post_meta( $product_id, "_{$key}_regular_price", $regular_price !== '' ? $regular_price * $value['exchange_rate'] : '' );							
				update_post_meta( $product_id, "_{$key}_sale_price", $sale_price !== '' ? $sale_price * $value['exchange_rate'] : '' );				
				update_post_meta( $product_id, "_{$key}_price", $price !== '' ? $price * $value['exchange_rate']  : '' );						
				
				// Update parent if grouped so price sorting works and stays in sync with the cheapest child				
				if ( $parent_id > 0 && $product->is_type('simple') ) {
					wcpbc_zone_grouped_product_sync( $key,  $parent_id );
				}
				
				do_action( 'wc_price_based_country_quick_or_bulk_edit_save_' . $product_type, $product, "_{$key}", $value );
			}						
		}
	}
	 ...
	 * Display Pro ad.
	 */
	public static function after_bookings_pricing() {
		self::show_upgrade_pro_notice( array( 'booking' ) );						
	}	
