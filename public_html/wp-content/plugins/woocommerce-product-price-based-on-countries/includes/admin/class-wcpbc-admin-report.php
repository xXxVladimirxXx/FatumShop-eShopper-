...
class WCPBC_Admin_Report {	
	...
	public static function report_notice() {
		...
		$currency_rates_string  = self::get_currency_rates_string();

		if ( $currency_rates_string && in_array( $screen, array( 'woocommerce_page_wc-reports', 'toplevel_page_wc-reports' ) ) && ( ! isset( $_GET['tab'] ) || in_array( $_GET['tab'], apply_filters('wc_price_based_country_tabs_report_notice', array( 'orders') ) ) ) ) {			
			
			echo '<div class="notice notice-info"><p>';
			printf( __( 'Totals in different currency to %s has been calculate by following exchange rates: %s', 'wc-price-based-country' ), $base_currency, $currency_rates_string ) ; 			
			echo '</p></div>';				
		}
	}
	...
	private static function get_currency_rates_string() {				
		$base_currency 		   = wcpbc_get_base_currency();
		$currency_rates_string = '';

		if ( $rates = self::get_currency_rates() ) {
			...
			$currency_rates_string = '<strong>' . implode( '</strong> - <strong>', $srates ) . '</strong>';	
		}

		return $currency_rates_string;		
	}
