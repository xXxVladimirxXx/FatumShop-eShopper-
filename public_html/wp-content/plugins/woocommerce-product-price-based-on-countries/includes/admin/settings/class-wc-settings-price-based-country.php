...
class WC_Settings_Price_Based_Country extends WC_Settings_Page {
	...
	public function get_settings() {
		$settings = apply_filters( 'wc_price_based_country_settings', array(
			...
			array(
				...
				'default'	=> 'no',
				'type' 		=> 'checkbox',
				'desc_tip' 	=> sprintf( __( 'This fired an AJAX request per page (%sread more%s).' ), '<a target="_blank" rel="noopener noreferrer" href="https://www.pricebasedcountry.com/docs/getting-started/geolocation-cache-support/?utm_source=settings&utm_medium=banner&utm_campaign=Docs">', '</a>' ) 
			),
			...
			array(
				...
				'default'	=> 'no',
				'type' 		=> 'checkbox', 
				'desc_tip' 	=> sprintf( __( 'Enable test mode to show pricing for a specific country (%sHow to test%s).', 'wc-price-based-country' ), '<a target="_blank" rel="noopener noreferrer" href="https://www.pricebasedcountry.com/docs/getting-started/testing/?utm_source=settings&utm_medium=banner&utm_campaign=Docs">', '</a>' ), 
			),
	...
	public function output_ads( $output ) {
		...
		<div class="wc-price-based-country-setting-wrapper-ads">
			<div class="wc-price-based-country-setting-content"><?php echo $output; ?></div>
			<div class="wc-price-based-country-setting-sidebar"><?php include( 'views/html-addons-banner.php' ); ?></div>
		</div>
		<?php
	}
	...
	public function save() {
		...
		} elseif( $current_section == 'zones' && ( ( ! empty( $_POST['action'] ) && $_POST['action'] == 'remove' ) || ( ! empty( $_POST['action2'] ) && $_POST['action2'] == 'remove' ) ) && isset( $_POST['region_key'] ) ) {
	...
	public function show_zone_messages() {
		global $current_section;
		if ( $current_section == 'zones' && ! empty( $_GET['edit_region'] ) && ! empty( $_GET['updated'] ) ) {				
			?>
			<div id="message" class="updated inline">
				...
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=price-based-country&section=zones' ) ); ?>">&larr; <?php _e( 'Back to Zones', 'wc-price-based-country' ); ?></a>					
	...
	private static function regions_save() {											
		...
		$region = self::get_regions_data( $region_key, wc_clean( $_POST ) ) ;
