	...
	public function column_name( $region ) {		
		...
		if ( $region['key'] !== $this->default_region_key ) {
			...
			$actions = array(
				...
				'trash' => '<a class="submitdelete wcpbc-delete-zone" title="' . esc_attr__( 'Remove zone', 'wc-price-based-country' ) . '" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'remove_region' => $region['key'] ), admin_url( 'admin.php?page=wc-settings&tab=price-based-country&section=zones' ) ), 'wc-price-based-country-remove-region' ) ) . '">' . __( 'Remove zone', 'wc-price-based-country' ) . '</a>'
			);
