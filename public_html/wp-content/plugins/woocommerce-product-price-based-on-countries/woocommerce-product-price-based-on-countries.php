...
class WC_Product_Price_Based_Country {
	...
	private function includes() {							
		...
		include_once( 'includes/class-wcpbc-pricing-zone.php' );				
		include_once( 'includes/class-wcpbc-pricing-zones.php' );				
		include_once( 'includes/class-wcpbc-integrations.php' );

		if ( $this->is_request( 'admin') ) {
			include_once( 'includes/admin/class-wcpbc-admin-notices.php' );												
			...
			include_once( 'includes/class-wcpbc-ajax-geolocation.php' );
		...
		if ( ( defined( 'DOING_CRON' ) || is_admin() ) && 'yes' === get_option( 'wc_price_based_country_allow_tracking', 'no' ) ) {			
			include_once( 'includes/class-wcpbc-tracker.php' );
		}			
	}
	...
	private function is_request( $type ) {
		...
		switch ( $type ) {
			...
			case 'frontend' :
				$is_heartbeat = $is_ajax && isset( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], array( 'heartbeat', 'get-comments', 'wp-remove-post-lock', 'wp-compression-test', 'wcs_update_one_time_shipping', 'wcs_product_has_trial_or_is_synced', 'woocommerce_add_attribute', 'woocommerce_save_attributes', 'woocommerce_link_all_variations', 'oembed-cache', 'woocommerce_do_ajax_product_export', 'woocommerce_do_ajax_product_import', 'woocommerce_json_search_products', 'woocommerce_json_search_products_and_variations', 'wc_price_based_country_load_country_pricing' ) );
	...
	public function frontend_init(){	 
		...
		if ( $this->current_zone = wcpbc_get_zone_by_country() ) {

			$this->customer = new WCPBC_Customer( $this->current_zone->get_data() ); // Legacy			
